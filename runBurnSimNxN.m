function [states,volume,surfaceArea,time,steps] = runBurnSimNxN(sizeN,coreMask,fixedMask,configArgs)
%runBurnSimNxN use the stepState function to run a simulation of cell decay
%   runBurnSimNxN uses the stepState function
...
% CONFIGS
%
%     wearRate is a positive decimal, where the wear deltas of each cell
%     are multiplied by this value. Increasing this accelerates the wear of
%     the simulation, decreasing this slows the wear of the simulation.
% 
...
%         toolkits to use. 
% 
%         "NONVECTOR" utilizes the most basic processing, this is the 
%         slowest by far, but may be useful for debugging purposes. I plan
%         to remove this, but I haven't had a reason to yet.
% 
% 
% OUTPUTS
...
configArgs.maxVal = sum(kernel,"all"); 

%Initialize the state table
stateTable = ones(sizeN,'single')*configArgs.defaultVal;
...
fixedVol = sum(fixedMask,"all")*configArgs.defaultVal;

% Sets all fixed values to the default val, in case of coreMask coincidence

stateTable(fixedMask) = configArgs.defaultVal;

%Create values for loop tracking

if configArgs.saveVideo
    states = zeros([size(stateTable), configArgs.estVals], 'like', stateTable); % Same type as input
else
    states = [];
end

outVals = zeros(2, configArgs.estVals);

volume = fixedVol + 1;
simtime = tic;
track = 1;

while round(volume) > fixedVol && track <= configArgs.timeout
    ...
    % Update simulation
    [stateTable, volume, surface] = stepState(stateTable, fixedMask, kernel, configArgs);
    outVals(1,track) = volume;
    outVals(2,track) = surface;
    
    track = track + 1;
    if configArgs.heartbeat
